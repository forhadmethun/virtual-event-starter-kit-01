...
export const PreviewScreen: React.FC<{ token: string }> = ({ token }) => {
  ...
  const { localPeer, audioEnabled, videoEnabled } = usePreview(
    token,
    "preview"
  );
  ...
  return (
    <div className={s["preview-container"]}>
      {localPeer ? (
        <PreviewVideo name={name} peer={localPeer} />
      ) : (
        <VideoLoader />
      )}
      <div className={s["wrapper"]}>
        ...
        <form onSubmit={joinRoom}>
          ...
          /> */}
          <p className={s["info"]}>
            <InfoIcon /> Note: Your mic is {audioEnabled ? "on" : "off"} and
            video is {videoEnabled ? "on" : "off"}
          </p>
...
const PreviewVideo: React.FC<{ peer: HMSPeer; name: string }> = ({
  peer,
  name,
}) => {
  const { color, initials } = getAvatarBg(name);
  const actions = useHMSActions();
  const { videoRef, isLocal, isAudioOn, isVideoOn, audioLevel } = useVideoTile(
    peer
  );
  return (
    <Preview.VideoRoot
